...
describe('Averages', function () {
  it('Moving Average of a sample serie', function () {
    ...
    const correctValues = [5, 7, 7.75, 9.5, 9];
    const movingAvg = tw.ma(serie, 4);
  ...
  it('MA accepts list as input', function () {
    const serie = [{ c: 2 }, { c: 6 }, { c: 5 }, { c: 7 }, { c: 10 }, { c: 9 }, { c: 12 }, { c: 5 }];
    const input = [2, 6, 5, 7, 10, 9, 12, 5];
    const result1 = tw.ma(serie, 4);
    const result2 = tw.ma(input, 4);
    assert.deepEqual(result1.map((r: any) => r.ma).slice(3), result2);
  });

  it('Exponential moving average test', function () {
    const series = [{ c: 64.75 }, { c: 63.79 }, { c: 63.73 }, { c: 63.73 }, { c: 63.55 },
    { c: 63.19 }, { c: 63.91 }, { c: 63.85 }, { c: 62.95 }, { c: 63.37 },
    { c: 61.33 }, { c: 61.51 }, { c: 61.87 }, { c: 60.25 }, { c: 59.35 },
    { c: 59.95 }, { c: 58.93 }, { c: 57.68 }, { c: 58.82 }, { c: 58.87 }];
    const expected = [63.682, 63.254, 62.937, 62.743, 62.290,
      61.755, 61.427, 60.973, 60.374, 60.092,
      59.870];
    const result = tw.ema(series, 10, ['c']);
    ...
    const result2 = tw.ema(series, 1, ['c']);
    for (let i = 0; i < result2.length; i++) {
      assert.equal(result2[i].ema.toFixed(3), series[i].c, 'EMA = 1, value ' + i + ' unchanged');
    }
  });

  it('EMA accepts list as input', () => {
    const series = [64.75, 63.79, 63.73, 63.73, 63.55,
    ...
    const expected = [63.682, 63.254, 62.937, 62.743, 62.290,
      61.755, 61.427, 60.973, 60.374, 60.092,
      59.870];
    assert.deepEqual(tw.ema(series, 10).map((v: any) => v.toFixed(3)), expected);
  });

  it('Weighted moving average test', function () {
    const series = [{ c: 1 }, { c: 2 }, { c: 3 }, { c: 4 }, { c: 5 }, { c: 6 }];
    const expected = [0.5, 0.83333, 1.16667, 1.5];
    const result = tw.wma(series, [0.6, 0.3, 0.1]);
    for (let i = 0; i < expected.length; i++) {
  ...
  it('WMA takes list as input', () => {
    ...
    const expected = [0.5, 0.83333, 1.16667, 1.5];
    const result = tw.wma(series, [0.6, 0.3, 0.1]);
    assert.deepEqual(result.map((v: any) => v.toFixed(5)), expected);
  });
});
