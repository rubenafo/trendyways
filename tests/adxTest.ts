...
describe('ADX', function () {
  ...
  const adxValues = tr.adx(data);

  it('returns a result array matching the input length', function () {
    assert.equal(adxValues.length, data.length);
  });

  it('index 14: first period with DI+, DI- and DX computed', function () {
    assert.equal(adxValues[14].di14p.toFixed(2), '6.66');
    assert.equal(adxValues[14].di14n.toFixed(2), '32.34');
    assert.equal(adxValues[14].dx.toFixed(2), '65.83');
    assert.equal(adxValues[14].adx, undefined, 'ADX is not yet defined before index 28');
  });

  it('index 28: first ADX value matches reference', function () {
    assert.equal(adxValues[28].adx.toFixed(2), '33.71');
    assert.equal(adxValues[28].di14p.toFixed(2), '22.73');
    assert.equal(adxValues[28].di14n.toFixed(2), '17.36');
    assert.equal(adxValues[28].dx.toFixed(2), '13.39');
  });

  it('index 35: ADX, DI+ and DI- continue to evolve correctly', function () {
    assert.equal(adxValues[35].adx.toFixed(2), '22.99');
    assert.equal(adxValues[35].di14p.toFixed(2), '27.92');
    assert.equal(adxValues[35].di14n.toFixed(2), '21.60');
  });

  it('index 37 (last): ADX value is correct', function () {
    assert.equal(adxValues[37].adx.toFixed(2), '21.59');
  });
});
